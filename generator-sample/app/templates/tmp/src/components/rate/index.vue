...
<script>
import Migrating from "../ele_utils/migrating";

import { removeClass, addClass, hasClass } from "@js/utils";
...
export default {
  ...
  methods: {
    addAnimation(index) {
      ...
      let ele = document.querySelector(`.el-rate__icon_${index}`);
      if (index != 0) {
        let ele_prev = document.querySelector(`.el-rate__icon_${index - 1}`);
        setTimeout(() => {
          removeClass(ele_prev, "transition_big");
        }, this.animationTime);
      }
      setTimeout(() => {
        addClass(ele, "transition_big");
        if (index < this.value) {
          this.addAnimation(index + 1);
        } else {
          removeClass(ele, "transition_big");
        }
      }, this.animationTime);
    },
    isObject(obj) {
    ...
    getValueFromMap(value, map) {
      const matchedKeys = Object.keys(map)
        .filter((key) => {
          const val = map[key];
          const excluded = this.isObject(val) ? val.excluded : false;
          return excluded ? value < key : value <= key;
        })
        .sort((a, b) => a - b);
      const matchedValue = map[matchedKeys[0]];
      return this.isObject(matchedValue)
        ? matchedValue.value
        : matchedValue || "";
    },
